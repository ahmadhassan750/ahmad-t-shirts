(() => {
  ...
  function init(){
    document.querySelectorAll('img').forEach((img,i)=>{
      img.addEventListener('error',()=>{if(img.dataset.fallback)return;img.dataset.fallback='1';img.src=(i%2?BACK:FRONT)},{once:true});
    ...
    if(!reduced()){
      document.querySelectorAll('.motion-card').forEach(el=>{
        el.addEventListener('pointermove',e=>{const r=el.getBoundingClientRect(),x=(e.clientX-r.left)/r.width-.5,y=(e.clientY-r.top)/r.height-.5;el.style.transform=`perspective(1100px) rotateX(${(-y*5).toFixed(2)}deg) rotateY(${(x*6).toFixed(2)}deg) translateY(-5px) translateZ(8px)`});
        el.addEventListener('pointerleave',()=>el.classList.contains('is-visible')&&(el.style.transform=''));
      });
    }
    if(!document.querySelector('.whatsapp-float')){
      const a=document.createElement('a');a.className='whatsapp-float';a.href=WA;a.target='_blank';a.rel='noopener';a.setAttribute('aria-label','Order on WhatsApp');a.innerHTML=`<span class="wa-logo">${waLogo}</span><span>ORDER ON WHATSAPP</span>`;document.body.appendChild(a);
    }
